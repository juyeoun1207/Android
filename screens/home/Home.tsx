...
const Home = ({ navigation }) => {
	...
	useEffect(() => {
		(async () => {
			try {
				console.log('start_screentime_usage')
				...
				if(stats?.weekly){
					...
					dayTypeList.forEach(dayType => {
						...
						if(stats.weekly[dayType]){
							time = stats.weekly[dayType].reduce((a,b) => a += Number(b.totalTimeInForeground || 0), 0)
						}
				...
				<View style={{paddingBottom:10, position:'absolute', zIndex:10, top:0, backgroundColor:"#fff", paddingTop:20, width:'100%', alignItems:'center', display:'flex', paddingLeft:'10%', paddingRight:'10%'}}>
					...
					<View style={{borderWidth:1, paddingTop:5, borderColor:'#000'}}>
						<View style={{marginBottom:10, alignItems:'center'}}>
							<CustomText>{`이번주 평균 사용시간 : ${weekList.filter(e => e.time > 0).length > 0 ? ((weekList.filter(e => e.time > 0).reduce((a,b) => a += Number(b.time || 0), 0) / weekList.filter(e => e.time > 0).length) / 3600).toFixed(1) : 0}H`}</CustomText>
						</View>
					...
					{/* <View style={{width:'100%',marginTop:10,display:'flex', height:200, backgroundColor:'lightgray'}}>
						...
						<Button 
							...
							onPress={() => {
								UsageMonitor.setUsageThreshold('com.android.chrome', 15) // 15초로 설정
					...
					<View style={{display:'flex',width:'100%'}}>
						<View style={{gap:15}}>
							{appList.map((data, index) => {
								...
								return (
									<Pressable key={index} onPress={() => {navigation.navigate('instagram')}} style={{...styles.appListBox, marginLeft:20, paddingRight:10, flexDirection: 'row', justifyContent:'space-between', alignItems: 'center', position:'relative'}}>
